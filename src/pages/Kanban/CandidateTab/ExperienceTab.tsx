...
export function ExperienceTab({
  ...
  const mockExperience: CandidateExperienceResponse = {
    id: candidate.id,
    summary:
      "Ứng viên có nền tảng phát triển backend & frontend, đã làm lead 1 nhóm nhỏ, có kinh nghiệm tích hợp hệ thống lớn.",
    totalYears: 6,
    previousRoles: [
      {
        company: "ABC Tech",
    ...
    timeline: [
      {
        id: "t1",
        title: "Gia nhập công ty ABC",
        description: "Bắt đầu role Senior",
        date: "2021-06-01",
        actor: "HR",
      },
      {
        id: "t2",
        title: "Được thăng chức leader",
        description: "Quản lý team 3 người",
        date: "2022-11-15",
        actor: "Manager",
      },
      {
        id: "t3",
        title: "Chuyển dự án lớn",
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const data = await candidateService.fetchExperience(
  ...
  return (
    <ScrollArea className="h-full px-5 pb-10 pt-5 sm:px-8">
      {isLoading ? (
        <LoadingSpinner
          message="Đang tải lịch sử kinh nghiệm..."
          variant="overlay"
          size="sm"
        />
      ) : (
          ...
          <div className="rounded-2xl border border-slate-100 bg-slate-50/80 px-4 py-3">
            ...
            {overviewExperiences?.totalYear ? (
              <p className="mt-1 text-xs text-slate-500">
                Tổng số năm kinh nghiệm: {overviewExperiences?.totalYear}
              </p>
            ) : null}
          </div>
      ...
      <div className="rounded-2xl border border-slate-100 bg-slate-50/80 p-6">
        ...
        <div className="mt-4 space-y-4">
          {overviewExperiences?.experiences &&
          overviewExperiences?.experiences.length > 0 ? (
